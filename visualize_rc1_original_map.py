#!/usr/bin/env python3
"""Visualize original (existing) RC1 connected components on an interactive Leaflet map."""
...
OUTPUT_HTML = "rc1_original_components_map.html"
TARGET_RC = 1
...
# --- 1. Load and filter to existing RC1 ---
print("Loading CSV...")
df = pd.read_csv(INPUT_CSV)
rc1 = df[df["existing_rc"] == TARGET_RC].copy()
print(f"Original RC1 rows: {len(rc1)}")
...
for edge_id, group in rc1.groupby("edge_id"):
    ...
    coords = list(zip(group["lat"].values, group["lon"].values))
    edges[edge_id] = {
        "coords": coords,
...
print(f"Original RC1 edges: {len(edges)}")
...
<b>Original RC1 Components</b><br>
"""
for i, cid in enumerate(size_rank[:10]):
